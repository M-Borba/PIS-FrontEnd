...
export default function ProjectTimeline({ onSwitch, isProjectView }) {
      ...
      .then((response) => {
        ...
        rows.map((proj) => {
          ...
          itemsToAdd.push({
            ...
            className: moment(proj.start_date).day() === 6 || moment(proj.start_date).day() === 0 ? "item-weekend" : "",
          });
        });
        setItems(itemsToAdd);
      })
  ...
  if (groups.length > 0 && !isProjectView && !fetchingError) {
        ...
        <Modal open={openInfo} onClose={handleInfoClose} disableEnforceFocus>
          <Box className={classes.modalInfo}>
            ...
            <InfoProyecto
              ...
              type={projectData.project_type?.replaceAll("_", " ").replace(/(^\w|\s\w)/g, (m) => m.toUpperCase())}
              state={projectData.project_state?.replace(/^\w/, (m) => m.toUpperCase())}
            />
          </Box>
        </Modal>
      </Fragment>
    ...
    return (
      <Box display="flex" flexDirection="column" alignItems="center">
        <img style={{ marginTop: "15%" }} className={classes.imgcontainer} src={not_found} />
        <Typography variant="h4" style={{ marginTop: "30px" }}>
